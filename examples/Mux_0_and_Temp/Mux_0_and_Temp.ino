...
/**---------------------------< REVISIONS >------------------------------------
2017Jul13			derived from 9548A Mux_and_temps.ino to test Wire1 aware library; supports temp sensor on mux channel 0 only
...
Systronix_TMP275 tmp275_mux0_48;
...
//  int16_t temp0;
  ...
  // Init tmp275_mux0_48 Sensors
  stat = tmp275_mux0_48.init(TMP275_CFG_RES12);
  if (SUCCESS != stat) Serial.printf(" tmp275_mux0_48 init failed with return of 0x%.2X: %s\r\n", tmp275_mux0_48.error.error_val);

  // leave the pointer set to read temperature
  stat = tmp275_mux0_48.pointer_write(TMP275_TEMP_REG_PTR);

  // Read Temp on sensors Mux Channel 0
  rawtemp=0;
  temp=0.0;
  stat = tmp275_mux0_48.register16_read (&rawtemp);
  if (stat != SUCCESS) Serial.printf("tmp275_mux0_48 error, stat=%u\r\n", stat);
  temp = tmp275_mux0_48.raw12_to_c(rawtemp);
  Serial.printf ("tmp275_mux0_48 %6.4f C (0x%.4X)\r\n", temp, rawtemp);
  
  tmp275_mux0_48.get_temperature_data();
  Serial.printf ("tmp275_mux0_48 temperature data:\n\t%6.4f C\n\t%6.4f F\n\t0x%.4X\n\t0x%.4X (H)\n\t0x%.4X (L)\r\n",
	tmp275_mux0_48.data.deg_c, tmp275_mux0_48.data.deg_f, tmp275_mux0_48.data.raw_temp, tmp275_mux0_48.data.t_high, tmp275_mux0_48.data.t_low);
